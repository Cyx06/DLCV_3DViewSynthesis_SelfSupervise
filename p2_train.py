...
def train(train_dataloader, validation_dataloader, model, device, args):
	...
	# by testing we know that gamma 0.9 is better than gamma 0.8 or gamma 0.85
	...
	for i in range(args.epoch):
		...
		total_loss = 0
		start = time()
		# about enumerate : https://www.runoob.com/python/python-func-enumerate.html
		...
		if accuracy >= max_accuracy:
			print('save model...')
			torch.save(model.state_dict(), os.path.join(args.trained_checkpoint))
...
def evaluate(validation_dataloader, model, device):
	...
	print('epochs ({}s) validation loss : {:.5f} , validation accuracy : {:.5f}'.format(int(end - start) , total_loss / len(os.listdir(args.validation_directory)) , count / len(os.listdir(args.validation_directory))))
...
def main(args):
	...
	print('train model...')
	model = ResNet50()
	model.load_state_dict(torch.load(os.path.join(args.pretrained_checkpoint), map_location = device))
