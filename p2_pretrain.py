...
def pretrain(pretrain_dataloader , model , device , args):
	...
	torch.save(model.state_dict() , os.path.join(args.pretrained_checkpoint))

def main(args):
	device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
	print('load data...')
	pretrain_image_name = load_data(args.pretrain_directory, None,'train')
	pretrain_dataloader = get_dataloader(args.pretrain_directory, pretrain_image_name, None, 'train', args.batch_size)
	print('train model...')
	model = ResNet50()
