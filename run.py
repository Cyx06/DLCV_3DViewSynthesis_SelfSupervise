...
def config_parser():
    ...
    parser.add_argument("--ft_path", type=str, default='./fine_last.tar',
    ...
    parser.add_argument("--TheJsonDir", default='')
    parser.add_argument("--TheOutputDir", default='')
...
def load_everything(args, cfg):
    ...
    '''
    data_dict = load_data(cfg.data, args.TheJsonDir)
    print('TheFileNames :', data_dict['TheFileNames'][0])
    # remove useless field
    kept_keys = {
            'hwf', 'HW', 'Ks', 'near', 'far', 'near_clip',
            'i_train', 'i_val', 'i_test', 'irregular_shape',
            'poses', 'render_poses', 'images', 'TheFileNames'}
    for k in list(data_dict.keys()):
        if k not in kept_keys:
            data_dict.pop(k)
    print("data_dict['i_val'] is :", data_dict['i_val'])
...
if __name__=='__main__':
    ...
    if args.render_test:
        ...
        rgbs, depths, bgmaps = render_viewpoints(
                ...
                **render_viewpoints_kwargs)
        for i in range(len(rgbs)):
            imageio.imwrite(os.path.join(args.TheOutputDir, data_dict['TheFileNames'][i]), utils.to8b(rgbs[i]))
        # imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=30, quality=8)
        # imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=30, quality=8)
